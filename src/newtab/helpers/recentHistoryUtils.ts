...
export function getBaseFilteredRecentItems(
  historyItems: RecentItem[]
): RecentItem[] {
  const res: RecentItem[] = [];
  ...
  historyItems.forEach((ri) => {
    if (ri.url && ri.title) {
      ...
      const hashesByPathName = deduplicatedHistoryItemsHashesByTitle.get(
        ri.title
      );
      if (!hashesByPathName) {
        deduplicatedHistoryItemsHashesByTitle.set(
          ri.title,
          new Set(urlObj.pathname)
        );
        res.push(ri);
      } else {
...
function mapHistoryToRecentAndFilterIrrelevant(
  ...
  list.forEach((item) => {
    ...
    if (
      item.url.includes("translate.google.") ||
      item.url.includes("www.deepl.com")
    ) {
      return;
    }
