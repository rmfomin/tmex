...
export function useSwipeAnimation(
  bookmarksRef: MutableRefObject<HTMLElement | null>,
  ...
  useEffect(() => {
    ...
    // support animation
    if (
      prevSpaceIdRef.current &&
      lastSwipeDirRef.current &&
      prevSpaceIdRef.current !== currentSpaceId
    ) {
      bookmarksRef.current.style.transform =
      ...
      requestAnimationFrame(() => {
        ...
        bookmarksRef.current!.style.transition =
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      const isMouseWheel =
        event.deltaMode === 1 || Math.abs(event.deltaY) >= 50; // Larger jumps
      ...
      if (Math.abs(event.deltaX) > Math.abs(event.deltaY)) {
        ...
        if (
          !timeoutId &&
          Math.abs(event.deltaX) - Math.abs(maxOfLastDeltas) > 3 &&
