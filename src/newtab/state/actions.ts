...
import {
  defaultStickerColor,
  stickerSizeM,
} from "../components/canvas/WidgetsHorMenu";
  ...
  switch (action.type) {
    ...
    case Action.CreateWidget: {
      ...
      const newWidget: IWidget = {
        ...
        widgetType: "Sticker",
        position:
          action.position ?? insertBetween(topWidget?.position ?? "", ""),
        pos: action.pos,
      ...
      return {
        ...
        spaces: state.spaces.map((space) =>
          space.id === action.spaceId
            ? {
                ...space,
                widgets: sortByPosition([...(space.widgets || []), newWidget]),
              }
            : space
        ),
        undoSteps,
    ...
    case Action.DeleteWidgets: {
      ...
      return {
        ...
        selectedWidgetIds: state.selectedWidgetIds.filter(
          (id) => !action.widgetIds.includes(id)
        ),
        spaces: updatedSpaces,
        undoSteps,
      };
    ...
    case Action.DuplicateWidgets: {
      const newWidgets = action.widgetIds
        .map((widgetId) => {
          const currentSpace = findSpaceById(state, state.currentSpaceId)!;
          const originalWidget = (currentSpace.widgets ?? []).find(
    ...
    case Action.SetEditingWidget: {
      return {
        ...
        selectedWidgetIds: action.widgetId
          ? [action.widgetId]
