 ...
 * вынесены отдельно, поэтому их можно тестировать без Zustand, React и Chrome.
 */
export function selectSpace(state: DashboardState, spaceId: number): DashboardState {
  ...
  return currentSpaceId === state.currentSpaceId ? state : { ...state, currentSpaceId };
}
...
export function createSpace(state: DashboardState, input: CreateSpaceInput): DashboardState {
  const space: SpaceV3 = {
    id: input.id,
    title: input.title,
    position: input.position ?? insertBetween(getLast(state.spaces)?.position ?? "", ""),
    objectType: "space",
    folders: [],
  };
  ...
  return {
    ...
    currentSpaceId: state.currentSpaceId === -1 ? input.id : state.currentSpaceId,
  };
}

export function updateSpace(state: DashboardState, spaceId: number, patch: SpacePatch): DashboardState {
  if (!state.spaces.some((space) => space.id === spaceId)) return state;
  return {
    ...state,
    spaces: sortByPosition(state.spaces.map((space) => (
      space.id === spaceId ? { ...space, ...patch } : space
    ))),
  };
}

export function moveSpace(state: DashboardState, input: MoveSpaceInput): DashboardState {
...
export function deleteSpace(state: DashboardState, spaceId: number): DashboardState {
  ...
  return {
    ...state,
    spaces,
    currentSpaceId: state.currentSpaceId === spaceId ? spaces[0]?.id ?? -1 : state.currentSpaceId,
  };
}
...
export function createFolder(state: DashboardState, input: CreateFolderInput): DashboardState {
  ...
  const folder: FolderV3 = {
    ...
    color: input.color,
    position: input.position ?? insertBetween(getLast(space.folders)?.position ?? "", ""),
    items: addItemsToFolder(input.items ?? [], []),
  };
  return updateSpace(state, space.id, { folders: sortByPosition([...space.folders, folder]) });
}

export function updateFolder(state: DashboardState, folderId: number, patch: FolderPatch): DashboardState {
  return updateFolderInState(state, folderId, (folder) => ({ ...folder, ...patch }), Boolean(patch.position));
}
...
export function deleteFolder(state: DashboardState, folderId: number): DashboardState {
  ...
  return space
    ? updateSpace(state, space.id, { folders: space.folders.filter((folder) => folder.id !== folderId) })
    : state;
}
...
export function moveFolder(state: DashboardState, input: MoveFolderInput): DashboardState {
  ...
  const targetSpace = state.spaces.find((space) => space.id === input.targetSpaceId);
  const folder = sourceSpace?.folders.find((candidate) => candidate.id === input.folderId);
  if (!sourceSpace || !targetSpace || !folder) return state;

  const spacesWithoutFolder = state.spaces.map((space) => (
    space.id === sourceSpace.id
      ? { ...space, folders: space.folders.filter((candidate) => candidate.id !== input.folderId) }
      : space
  ));
  const targetWithoutFolder = spacesWithoutFolder.find((space) => space.id === input.targetSpaceId)!;
  const movedFolder = {
    ...folder,
    position: getInsertionPosition(targetWithoutFolder.folders, input.insertBeforeFolderId),
  };
  return {
    ...state,
...
export function createFolderItem(state: DashboardState, input: CreateFolderItemInput): DashboardState {
...
export function createFolderItems(state: DashboardState, input: CreateFolderItemsInput): DashboardState {
  ...
  if (!folder || (input.targetGroupId && input.items.some(isGroup))) return state;
  const items = input.targetGroupId
    ? addItemsToGroup(
        input.items.filter((item): item is FolderItemToCreate | BookmarkItemV3 => !isGroup(item)),
        folder.items,
        input.targetGroupId,
        input.insertBeforeItemId,
      )
    : addItemsToFolder(input.items, folder.items, input.insertBeforeItemId);
  return updateFolderInState(state, folder.id, (current) => ({ ...current, items }));
}

export function updateFolderItem(state: DashboardState, itemId: number, patch: FolderItemPatch): DashboardState {
...
export function moveFolderItems(state: DashboardState, input: MoveFolderItemsInput): DashboardState {
  const items = input.itemIds.map((itemId) => findAnyItemById(state.spaces, itemId));
  ...
  if (!targetFolder || (input.targetGroupId && !targetGroupExists)) return state;
...
function updateFolderInState(
  ...
  const space = findSpaceByFolderId(state.spaces, folderId);
  if (!space) return state;
  const folders = space.folders.map((folder) => (folder.id === folderId ? update(folder) : folder));
  return updateSpace(state, space.id, { folders: sortFolders ? sortByPosition(folders) : folders });
}

function findSpaceByFolderId(spaces: SpaceV3[], folderId: number): SpaceV3 | undefined {
  return spaces.find((space) => space.folders.some((folder) => folder.id === folderId));
}

function findFolderById(spaces: SpaceV3[], folderId: number): FolderV3 | undefined {
  return findSpaceByFolderId(spaces, folderId)?.folders.find((folder) => folder.id === folderId);
}

function findFolderByItemId(spaces: SpaceV3[], itemId: number): FolderV3 | undefined {
  return spaces.flatMap((space) => space.folders).find((folder) => (
    folder.items.some((item) => item.id === itemId || (
      item.type === "group" && item.groupItems.some((child) => child.id === itemId
      )))
  ));
}

function findAnyItemById(spaces: SpaceV3[], itemId: number): ItemV3 | undefined {
  for (const folder of spaces.flatMap((space) => space.folders)) {
    for (const item of folder.items) {
      if (item.id === itemId) return item;
      if (item.type === "group") {
        const child = item.groupItems.find((groupItem) => groupItem.id === itemId);
...
function addItemsToFolder(
  ...
  const beforeIndex = existingItems.findIndex((item) => item.id === insertBeforeItemId);
  let previous = existingItems[beforeIndex === -1 ? existingItems.length - 1 : beforeIndex - 1];
  const next = existingItems[beforeIndex];
  const newItems = insertingItems.map((insertingItem) => {
    const item = normalizeItem(insertingItem, insertBetween(previous?.position ?? "", next?.position ?? ""));
    previous = item;
    return item;
  });
...
function addItemsToGroup(
  ...
  return existingItems.map((item) => (
    item.type === "group" && item.id === targetGroupId
      ? { ...item, groupItems: addItemsToFolder(insertingItems, item.groupItems, insertBeforeItemId) as BookmarkItemV3[] }
      : item
  ));
}
...
function removeItemFromFolderItems(items: ItemV3[], itemId: number): ItemV3[] {
  return items.flatMap((item) => {
    ...
    if (item.type !== "group" || !item.groupItems.some((child) => child.id === itemId)) return [item];
    return [{ ...item, groupItems: item.groupItems.filter((child) => child.id !== itemId) }];
  });
}

function getInsertionPosition<T extends { id: number; position: string }>(items: T[], insertBeforeId?: number): string {
  const beforeIndex = items.findIndex((item) => item.id === insertBeforeId);
  const previous = items[beforeIndex === -1 ? items.length - 1 : beforeIndex - 1];
  return insertBetween(previous?.position ?? "", items[beforeIndex]?.position ?? "");
}
