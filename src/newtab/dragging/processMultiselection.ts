...
import {
  ...
  WidgetInfo,
} from "../components/canvas/widgetsSelectionFrame";
import {
  areRectsOverlapping,
  normalizeRect,
  uniteRects,
...
export function processMultiselection(
  ...
  const itemElements = Array.from(
    document.querySelectorAll(".bookmarks .folder-item__inner")
  ) as HTMLElement[];
  ...
  const widgetsElements = Array.from(
  ...
  const onMouseMove = (e: MouseEvent, mouseMoved: boolean) => {
    ...
    const selectedWidgetsInfos = allWidgetsInfos.filter((w) =>
      areRectsOverlapping(selectionRect, w.rect)
    );
    ...
    if (selectedWidgetsInfos.length > 0) {
      ...
      if (selectedWidgetsInfos.length > 1) {
        ...
        const selectionRect = uniteRects(
          selectedWidgetsInfos.map((w) => w.rect)
        );
  ...
  const onMouseUp = (e: MouseEvent, mouseMoved: boolean) => {
    if (!mouseMoved) {
      ...
      const folderItemFirstSelected = document.querySelector(
        ".folder-item--first-selected"
      );
