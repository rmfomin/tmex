...
export function processWidgetsDragAndDrop(
  mouseDownEvent: React.MouseEvent,
  widgetsConfig: PConfigWidgets
) {
  let selectedWidgets = Array.from(
    document.querySelectorAll<HTMLElement>(".widget.selected")
  );
  let selectedWidgetIds = getIdsFromElements(selectedWidgets);
  let movedWidgetIds = new Set<number>();
  const initWidgetPositions = Array.from(
  ...
  const onMouseUp = (e: MouseEvent, mouseMoved: boolean) => {
    if (mouseMoved) {
      const res = Array.from(movedWidgetIds).map((id) => ({
        id: id,
        pos: getPosFromElement(
          document.querySelector(`.widget[data-id="${id}"]`)!
        ),
      }));
      ...
      if (e.shiftKey) {
        if (selectedWidgetIds.includes(targetWidgetId)) {
          ...
          selectedWidgetIds = selectedWidgetIds.filter(
