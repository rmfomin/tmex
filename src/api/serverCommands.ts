...
export function executeAPICall(
  command: APICommandPayloadFull,
  dispatch: ActionDispatcher
) {
  const dispatchApiResolved = () =>
    dispatch({ type: Action.APICommandResolved, commandId: command.commandId });
        ...
        .then((value) => {
          if (command.type === Action.CreateFolder) {
            dispatch({
              ...
              entityType: "folder",
              localId: command.body.folder.id!,
              remoteId: value.id,
            });
          }
